...
int seqNum[100] = {0};
int blockSize = 8;
int nodeBlockSize[100] = {0};
int BAckBlockSize[100] = {0};
int oldestsqn[100][100] = {0};
int const sqnHistoryBuffSize = 80;
int pointer[100][100] = {0};
int sqnHistory[100][100][sqnHistoryBuffSize];
bool BACKRecieved[100] = {0};
int frameStatusBitMap[100]={};

void BaselineBANMac::push2(int sqn,int src,int des)
{
	int temp;
	if (pointer[src][des] == sqnHistoryBuffSize-1)
	{
		for (int i = sqnHistoryBuffSize; i > 0; i--)
		{
			temp = sqnHistory[src][des][i-1];
			sqnHistory[src][des][i] = sqn;
			if (i-2>=0)
			{
				sqnHistory[src][des][i-2] = temp;
			}
		}
	...
	{
		sqnHistory[src][des][pointer[src][des]] = sqn;
		pointer[src][des]++;
//		trace()<<"what has been pushed: "<< sqn<<":and this is in the pointer "<<pointer[src][des]<<"and this is in the buffer"<< sqnHistory[src][des][pointer[src][des]-1]<<","<<sqnHistory[src][des][pointer[src][des]-2];
	} 
}
...
void BaselineBANMac::timerFiredCallback(int index) {
	switch (index) {
		...
		case ACK_TIMEOUT: {
			...
			int maxPacketTriesModified = maxPacketTries;
			// check if we reached the max number and if so delete the packet
			if (packetToBeSent->getAckPolicy() == B_ACK)
			{
				maxPacketTriesModified = maxPacketTries + 4;
			}
			
			if (currentPacketTransmissions + currentPacketCSFails == maxPacketTriesModified) {
				trace()<<"A Packet was dropped after:" <<maxPacketTriesModified<<":tries:Pckt sqn:"<<packetToBeSent->getSequenceNumber()<<":src:"<<packetToBeSent->getSource()<<":des:"<<packetToBeSent->getDestination();
				// collect statistics
...
void BaselineBANMac::fromNetworkLayer(cPacket *pkt, int dst) {
	...
	if(!isHub)
	{
		...
		BaselineBANDataPkt->setSource(SELF_MAC_ADDRESS);
		BaselineBANDataPkt->setDestination(connectedHID);
		BaselineBANDataPkt->setNID(connectedNID);
		BaselineBANDataPkt->setSequenceNumber(seqNum[SELF_MAC_ADDRESS]);
		if(seqNum[SELF_MAC_ADDRESS]<254)
		{
			seqNum[SELF_MAC_ADDRESS]++;
		}
		else
		{
			seqNum[SELF_MAC_ADDRESS] = 0;
		}

		encapsulatePacket(BaselineBANDataPkt,pkt,6);
	...
	if (bufferPacket(BaselineBANDataPkt)) {
		//trace()<<"before TX attempt PKTNID is: "<<BaselineBANDataPkt->getNID();
		attemptTxInRAP();
	} else {
...
void BaselineBANMac::fromRadioLayer(cPacket *pkt, double rssi, double lqi) {
    ...
    bool forMe = isPacketForMe(BaselineBANPkt);
	//trace() << "ACK_policy type: "<< BaselineBANPkt->getAckPolicy();
	/* Handle data packets */
	if(BaselineBANPkt->getFrameSubtype() != BEACON)
		trace()<<"A Packet recieved from src:"<<BaselineBANPkt->getSource()<<":des:"<<BaselineBANPkt->getDestination()<<":sqn:"<<BaselineBANPkt->getSequenceNumber()<<":at node:"<< SELF_MAC_ADDRESS <<":RSSI:"<<rssi<<":for me:"<<forMe<<":ACK Policy:"<<BaselineBANPkt->getAckPolicy();
	...
	if (BaselineBANPkt->getFrameType() == DATA) {
		...
		if (BaselineBANPkt->getAckPolicy() == L_ACK_POLICY)
		{
			recieved[BaselineBANPkt->getSource()]++;
			push2(BaselineBANPkt->getSequenceNumber(),BaselineBANPkt->getSource(),SELF_MAC_ADDRESS);
		...
		if (BaselineBANPkt->getAckPolicy() == B_ACK_POLICY)
		{
			...
			BaselineBAckPacket * BAckPkt = new BaselineBAckPacket("B-ACK packet",MAC_LAYER_PACKET);
			// next block size and 
			if(!BACKRecieved[BaselineBANPkt->getSource()])
			{
				BACKRecieved[BaselineBANPkt->getSource()] = 1;
				// max block size 
				BAckPkt->setNextBlockSize(blockSize);
				BAckPkt->setOldestFrameExpected(BaselineBANPkt->getSequenceNum()+1);
				BAckPkt->setFrameStatusBitmap(255);			
			}
			else
			{
				int frameStatusBitMap =0;
				int  myBlockSize = 0;
				int myOldestSqn = sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1];
				int diff =  BaselineBANPkt->getSequenceNumber() - myOldestSqn;
				// while ((oldestsqn[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS] != sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1])) 
				// {
				// 	if(diff != 1)
				// 	{
				// 		for (int i = 0; i < diff-1; i++)
				// 		{
				// 			frameStatusBitMap = frameStatusBitMap << 1;
				// 			myBlockSize++;
				// 		}
				// 	}
				// 	else
				...
				// 	}
				// 	trace()<<"src "<<BaselineBANPkt->getSource()<<" sqn "<<BaselineBANPkt->getSequenceNumber()<<" last BACK sqn "<<oldestsqn[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]<<" top of the buff now"<< sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1]<<" FSB "<<frameStatusBitMap<<"myBlockSize:"<<myBlockSize<<"diff:"<<diff;

				// 	if (myBlockSize > 7)
				// 	{
				// 		trace()<<"#B-ACK that has been missed: "<< floor(myBlockSize/8) << ": from node: "<<BaselineBANPkt->getSource();
				// 		while (oldestsqn[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS] != sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1])
				// 			{
				// 				pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]--;
				// 			}

				// 		break;
				// 	}
				// 	diff = sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1] - sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-2] ;
				// 	pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]--;
				...
				BAckPkt->setSource(SELF_MAC_ADDRESS);
				BAckPkt->setDestination(BaselineBANPkt->getSource());
				if( myBlockSize > 0)//if packet is missed 
				{
					BAckPkt->setNextBlockSize(8);
					BAckPkt->setOldestFrameExpected(myOldestSqn);
					BAckPkt->setFrameStatusBitmap(255);			
				}
				else
				{
					frameStatusBitMap = frameStatusBitMap >> 1;
					BAckPkt->setNextBlockSize(blockSize);
					BAckPkt->setOldestFrameExpected(BaselineBANPkt->getSequenceNumber()+1);
					BAckPkt->setFrameStatusBitmap(255);
				}
			}
			oldestsqn[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS] = BaselineBANPkt->getSequenceNumber();
			push2(BaselineBANPkt->getSequenceNumber(),BaselineBANPkt->getSource(),SELF_MAC_ADDRESS);
			

			setHeaderFields(BAckPkt,N_ACK_POLICY,CONTROL, B_ACK );
			BAckPkt->setNID(BaselineBANPkt->getNID());
			BAckPkt->setByteLength(BASELINEBAN_HEADER_SIZE);
			BAckPkt->setDestination(BaselineBANPkt->getSource());
			BAckPkt->setSource(SELF_MAC_ADDRESS);
			BAckPkt->setSequenceNumber(seqNum[SELF_MAC_ADDRESS]);
			
			seqNum[SELF_MAC_ADDRESS]++;
			if (seqNum[SELF_MAC_ADDRESS] > 255)
			{
				seqNum[SELF_MAC_ADDRESS] =0;
			}

			// int *temp = sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS];
			// trace()<<*(temp+pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1)<<","<<sqnHistory[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS][pointer[BaselineBANPkt->getSource()][SELF_MAC_ADDRESS]-1];

			
			//BAckBlockSize[BaselineBANPkt->getSource()] = BaselineBANPkt->getNextBlockSize();


			trace() << "transmitting B_ACK with src:"<<BAckPkt->getSource() <<":des:"<<BAckPkt->getDestination()<<":sqn:"<< BAckPkt->getSequenceNumber()<<":oldest expected:" <<BAckPkt->getOldestFrameExpected() <<":recieeved:"<<recieved[BaselineBANPkt->getSource()] <<":out of:" << nodeBlockSize[BAckPkt->getDestination()]<<":ACK Policy:"<<BAckPkt->getAckPolicy();
			recieved[BaselineBANPkt->getSource()] = 0;
			toRadioLayer(BAckPkt);
			toRadioLayer(createRadioCommand(SET_STATE,TX)); isRadioSleeping = false;
			/* Any future attempts to TX should be done AFTER we are finished TXing
			* the B-ACK. To ensure this we set the appropriate timer and variable.
	...
	if (BaselineBANPkt->getAckPolicy() == I_ACK_POLICY) {
		...
		ackPacket->setDestination(BaselineBANPkt->getSource());
		...
		if (sendIAckPoll) {
			...
			if (!naivePollingScheme) {
				...
				if (hubPollTimers.empty() || hubPollTimers.back().NID != BaselineBANPkt->getNID() ) {
					// trace() << "TEST: frameStartTime= "<<frameStartTime<<
					// " poll from start= "<<(nextFuturePollSlot-1)*allocationSlotLength<<
					// " timer= "<<frameStartTime + (nextFuturePollSlot-1)*allocationSlotLength - getClock();
					if (hubPollTimers.empty())
		...
		trace() << "transmitting I-ACK to/from NID:" << BaselineBANPkt->getNID();
	...
	switch(BaselineBANPkt->getFrameSubtype()) {
		...
		case B_ACK: {
			BaselineBAckPacket *BAckPkt = check_and_cast<BaselineBAckPacket*>(BaselineBANPkt);
			waitingForACK = false;
			cancelTimer(ACK_TIMEOUT);
			cancelAndDelete(packetToBeSent);
			packetToBeSent = NULL;
			...
			currentPacketCSFails = 0;
			CW = CWmin[priority];
			nodeBlockSize[SELF_MAC_ADDRESS] = BAckPkt->getNextBlockSize();
			trace()<<"BACK recieved with src:"<<BAckPkt->getSource()<<":des:"<<SELF_MAC_ADDRESS<<":sqn:"<<BAckPkt->getSequenceNumber()<<":Frame status Bitmap:"<<BAckPkt->getFrameStatusBitmap()<<":blck size:"<<nodeBlockSize[SELF_MAC_ADDRESS];
...
bool BaselineBANMac::isPacketForMe(BaselineMacPacket *pkt) {
	...
	if (connectedHID == pktHID) 
	{
		...
		if ((connectedNID == pktNID) || (pktNID == BROADCAST_NID) ) 
		{
			if(pkt->getFrameType() == DATA)
			{
				// trace() << "pktHID=" << pktHID << ", pktNID=" << pktNID 
				// << ", connectedHID=" << connectedHID << ", connectedNID=" 
				// << connectedNID <<", SenderID="<< pkt->getSource()
				// <<", des="<<pkt->getDestination();
			}
			return true;
		}
...
void BaselineBANMac::attemptTX() {
	...
	else if (connectedNID != UNCONNECTED && TXBuffer.size() != 0) 
	{
		...
		//set the header based on ACK policy (L_ACK or B_ACK)
		blockSend(packetToBeSent,nodeBlockSize[SELF_MAC_ADDRESS]);

		//trace()<<"sqn: "<<packetToBeSent->getSequenceNumber()<<":src: "<<SELF_MAC_ADDRESS<<":ACK Policy:"<<packetToBeSent->getAckPolicy()<<":Des:"<<packetToBeSent->getDestination();
		trace()<<"Sent from src: "<<packetToBeSent->getSource()<<":des: "<<packetToBeSent->getDestination()<<":sqn:"<<packetToBeSent->getSequenceNumber()<<":ACK:"<<packetToBeSent->getAckPolicy();
...
void BaselineBANMac::sendPacket() {
	...
	attemptingToTX = false;
	packetToBeSent->setSource(SELF_MAC_ADDRESS);
	...
	if (packetToBeSent->getAckPolicy() == I_ACK_POLICY || packetToBeSent->getAckPolicy() == B_ACK_POLICY) {
		 ...
		 */
		//trace() << "TXing[" << packetToBeSent->getName() << "], ACK_TIMEOUT in ";
		setTimer(ACK_TIMEOUT, 
		(SLEEP2TX + TX_TIME(
			packetToBeSent->getByteLength()) + 2*pTIFS + TX_TIME(BASELINEBAN_HEADER_SIZE)) * (1 + mClockAccuracy));
		waitingForACK = true;
...
void BaselineBANMac::blockSend(BaselineMacPacket *pkt,int blockSize)
{
	if(cnt[SELF_MAC_ADDRESS] == 0)
	{
		setHeaderFields(pkt, B_ACK_POLICY, DATA, B_ACK);
	}
	else
	{
		setHeaderFields(pkt, L_ACK_POLICY, DATA, RESERVED);
	}

	if(cnt[SELF_MAC_ADDRESS] == blockSize)
	{
		cnt[SELF_MAC_ADDRESS] = 0;
	}
	else
	{
		cnt[SELF_MAC_ADDRESS] ++;
	}
}